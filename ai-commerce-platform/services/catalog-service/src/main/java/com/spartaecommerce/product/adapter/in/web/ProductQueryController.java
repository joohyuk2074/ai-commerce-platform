...
public class ProductQueryController {
  ...
  @Operation(summary = "상품 복수 조회", description = "상품 ID 목록으로 여러 상품 정보를 한 번에 조회합니다.")
  @ApiResponses({
      @ApiResponse(
          responseCode = "200",
          description = "조회 성공",
  ...
  public ResponseEntity<CommonResponse<List<ProductResponse>>> getProducts(
      @Parameter(description = "조회할 상품 ID 목록", required = true)
      @RequestBody List<Long> productIds
  ) {
    List<ProductResult> productResults = productQueryUseCase.getProducts(productIds);
    List<ProductResponse> productResponses = productResults.stream()
        .map(ProductResponse::from)
        .toList();

    CommonResponse<List<ProductResponse>> commonResponse = CommonResponse.success(productResponses);
    return ResponseEntity.ok(commonResponse);
  }
